...
puzzle_solver_path=os.path.join(os.path.expanduser('~') , 'PycharmProjects' , 'deepproblog_pieter' , 'src','deepproblog','puzzle_solver_simple_v2')
...
class shape_colour_dataset(Dataset,TorchDataset):
    def __init__(self, subset):
        ...
        while i<len(self.labels):
                self.data.append((self.labels.iloc[i,0], self.labels.iloc[i,1], self.labels.iloc[i,2],
                                 self.labels.iloc[i,3], self.labels.iloc[i,4]))
                i+=1
    def __len__(self):
    ...
    def to_query(self, i: int) -> Query:
        ...
        return Query(Term("solution", *(x for x in tensor_vars),
                          Constant(self.data[i][3]),Constant(self.data[i][4])),
                     subs, output_ind=(-1,-2))
